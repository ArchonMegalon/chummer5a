...
public sealed class InProcessAvaloniaCoachSidecarClient : IAvaloniaCoachSidecarClient
{
    ...
    public InProcessAvaloniaCoachSidecarClient(IAiGatewayService aiGatewayService)
    {
        _aiGatewayService = aiGatewayService;
    }

    public Task<AvaloniaCoachSidecarCallResult<AiGatewayStatusProjection>> GetStatusAsync(CancellationToken ct = default)
        => Task.FromResult(FromAiResult(_aiGatewayService.GetStatus(OwnerScope.LocalSingleUser)));

    public Task<AvaloniaCoachSidecarCallResult<AiProviderHealthProjection[]>> ListProviderHealthAsync(string? routeType = null, CancellationToken ct = default)
        => Task.FromResult(FromAiListResult(_aiGatewayService.ListProviderHealth(OwnerScope.LocalSingleUser)));
    ...
    public Task<AvaloniaCoachSidecarCallResult<AiConversationAuditCatalogPage>> ListConversationAuditsAsync(
        ...
        => Task.FromResult(
            FromAiResult(
                _aiGatewayService.ListConversationAudits(
    ...
    private static AvaloniaCoachSidecarCallResult<T> FromAiResult<T>(AiApiResult<T> result)
    {
        ...
        {
            return AvaloniaCoachSidecarCallResult<T>.Failure(0, "Coach request returned without a payload.");
        }

        return AvaloniaCoachSidecarCallResult<T>.Success(200, result.Payload);
    }

    private static AvaloniaCoachSidecarCallResult<TItem[]> FromAiListResult<TItem>(AiApiResult<IReadOnlyList<TItem>> result)
    {
        if (result.NotImplemented is not null)
        {
            return AvaloniaCoachSidecarCallResult<TItem[]>.FromNotImplemented(501, result.NotImplemented);
        }

        if (result.QuotaExceeded is not null)
        {
            return AvaloniaCoachSidecarCallResult<TItem[]>.FromQuotaExceeded(429, result.QuotaExceeded);
        }

        if (result.Payload is null)
        {
            return AvaloniaCoachSidecarCallResult<TItem[]>.Failure(0, "Coach request returned without a payload.");
        }

        return AvaloniaCoachSidecarCallResult<TItem[]>.Success(200, [.. result.Payload]);
    }
}
