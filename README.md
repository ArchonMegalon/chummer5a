...
* `/api/runtime/locks/*` exposes a reusable runtime-lock catalog that now merges owner-scoped persisted runtime locks with the current profile-derived entries so saved, installed, pinned, published, and derived runtime fingerprints have an explicit registry path instead of living only inside profile detail payloads. Runtime-lock catalog rows and hub install previews surface owner install state before mutation endpoints land.
