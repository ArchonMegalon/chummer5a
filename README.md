...
* `/api/hub/publish/*` and `/api/hub/moderation/*` now exist as dedicated protected seams for future ChummerHub draft/submit/review workflows. They currently return explicit `hub_publication_not_implemented` receipts through shared application services so publication and moderation can evolve independently from the read-only catalog/install-preview surface.
