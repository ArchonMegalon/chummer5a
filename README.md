...
* `/api/hub/publish/*` and `/api/hub/moderation/*` now act as dedicated protected ChummerHub publication seams with owner-scoped persisted draft and moderation state. The current slice supports draft create/list, submit-for-review, and owner queue inspection while the broader multi-user registry/reviewer model continues to deepen behind the same application contracts.
