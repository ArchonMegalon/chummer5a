...
* `/api/hub/search` and `/api/hub/projects/*` are the first ChummerHub-style read-only discovery/detail surfaces. They already aggregate RulePacks, RuleProfiles, BuildKits, and runtime locks through one shared browse/query contract so hub-style discovery does not depend on head-specific catalog composition.
