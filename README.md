...
* `/api/hub/search`, `/api/hub/projects/*`, and `/api/hub/projects/*/install-preview` are the first ChummerHub-style discovery/detail/install-preview surfaces. They already aggregate RulePacks, RuleProfiles, BuildKits, and runtime locks through shared browse/query and install-preview contracts so hub-style discovery does not depend on head-specific catalog composition.
