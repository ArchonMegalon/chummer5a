...
using Chummer.Coach.Web.Components;

var builder = WebApplication.CreateBuilder(args);
string? configuredPathBase = builder.Configuration["Chummer:PathBase"];
string? environmentPathBase = Environment.GetEnvironmentVariable("CHUMMER_COACH_PATH_BASE");
PathString pathBase = NormalizePathBase(configuredPathBase ?? environmentPathBase);

builder.Services.AddRazorComponents()
    .AddInteractiveServerComponents();
builder.Services.AddScoped<BrowserCoachApiClient>();
