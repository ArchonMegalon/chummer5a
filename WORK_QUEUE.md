...
| WQ-003 | in_progress | P2 | Continue zero-risk warning reduction from dominant buckets. | agent | 2026-03-05 progress: cleared `CA1875` and `MSTEST0001`, fully eliminated `CS8632` and `CA1860`, and reduced `MSTEST0037` in several assertion-conversion batches. Latest `docker-tests` run passed (`390/390`) with `108 Warning(s)`, `0 Error(s)`; dominant residual bucket is `MSTEST0037`. |
...
| WQ-008 | queued | P2 | Continue `MSTEST0037` reduction batch 4 with collection-count modernizations. | agent | Target safe conversions still flagged in top files (`CharacterSectionServiceTests`, `ContentOverlayCatalogServiceTests`, `BlazorShellComponentTests`) and re-run `bash scripts/runbook.sh docker-tests`. |
